...
Extract the downloaded image where necessary.
Write the image to a USB drive using balenaEtcher.
Boot the firewall appliance from the USB drive.
Install pfSense on the hardware appliance.
Assign the network interfaces.
Configure the WAN and LAN networks.
...
The WireGuard configuration was based on the Proton VPN documentation:
...
Network and security through pfSense
Intrusion detection and traffic monitoring through Snort
Virtualization and services through Proxmox and Debian containers
Local DNS and ad blocking through Pi-hole
Centralized storage and file sharing through FSRV and Samba
